...
int isPalindrome(Stack* s)
{
	...
	charNode* HelpStack = (charNode*)malloc(sizeof(charNode)); // עזר
	...
	int namLet = 0;
	int count = 0;
	Top->head = (s->head);   	//טופ מצביע לראש המילה 
	HelpStack->next = NULL;
	...
	Top->head = s->head;
	Top2->head = s->head;
	for (count; count > 0; count--)			//לעבור אות מההתחלה ואות מהסוף
	{
		char letter = (char)malloc(sizeof(char)); if (letter == NULL)return(0);

		letter = givLeter(&Top2->head);		// מביא אות אות 

		for (int i = count; i > 0; i--) {
			Top->head = Top->head->next;  // אות מהסוף
		...
		Top->head = s->head;
		Top2->head = s->head;
		for (namLet; namLet != 0; namLet--) { // מצביע על האות ההבאה במחבנית
...
char givLeter(charNode* Letter)      //מוציאה אות מאיבר ברשימה.מחסנית 
{
	charNode* Letter2 = (charNode*)malloc(sizeof(charNode));
	if (Letter2 == NULL) //בדיאה אם ההקצאה נכשלה
	{
		printf("no memory!!\n");
		return(0);
	}
